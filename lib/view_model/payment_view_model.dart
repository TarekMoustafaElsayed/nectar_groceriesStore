...
class PaymentViewModel extends GetxController {
  ...
  void servicecCallList() {
    ...
    Globs.showHUD();
    ServiceCall.post({}, SVKey.svPaymentMethodList, isToken: true, withSuccess: (resObj) async {
      Globs.hideHUD();
  ...
  void serviceCallRemove(PaymentModel pObj) {
    ...
    ServiceCall.post({"pay_id": (pObj.payId ?? 0).toString()},
        SVKey.svRemovePaymentMethod, isToken: true, withSuccess: (resObj) async {
          Globs.hideHUD();
  ...
  void serviceCallAdd(VoidCallback didDone) {
    ...
    if(txtCardNumber.value.text.length != 16) {
      Get.snackbar(Globs.appName, "Please enter a valid card number");
      return;
    }

    if(txtMonth.value.text.isEmpty) {
      Get.snackbar(Globs.appName, "Please enter your card month");
      return;
    }

    if(txtYear.value.text.isEmpty) {
      Get.snackbar(Globs.appName, "Please enter your card year");
      return;
    }
